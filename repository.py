...
class Repository:
    ...
    def create_book(self, cursor, book:Book):
        query = """INSERT INTO book (title, author, grade, is_finished, opinion_id, genre) 
                                         VALUES (%s,%s,%s,%s,%s,%s)"""
        query_args = (book.title, book.author, book.grade, book.is_finished, book.opinion_id, book.genre)
        cursor.execute(query, query_args)
    ...
    # @cursor_handler
    # def ubdate_book(self, cursor, id, title, author, genre, grade, opinion, opinion_id, is_finished):
    #     query = """UPDATE book SET is_finished = %s   
    #                          WHERE id= %s """
    #     query_args = (title, id, title, author, genre, grade, opinion, opinion_id, is_finished)
    #     cursor.execute(query, query_args)
